...
class Conversation(db.Model):
    ...
    other_user_id = db.Column(db.Integer, nullable=False)
...
def contact():
    ...
    return render_template('contact_list.html', conversations=conversations, form=form)
...
def view_contact(other_user_id):
    ...
    return render_template('view_contact.html', conversation=conversation)
...
def start_new_conversation():
    ...
    # 会話を開始するユーザーを選択（選択ロジックをカスタマイズできます）
    if other_users:
        ...
        existing_conversation = Conversation.query.filter(
            ...
            ((Conversation.user2_id == current_user.id) & (Conversation.user1_id == other_user.id))
        ).first()

        # デバッグログ
        print(f"Existing Conversation: {existing_conversation}")

        if not existing_conversation:
            # 新しい会話を作成
            new_conversation = Conversation(user1_id=current_user.id, user2_id=other_user.id)
